...
/**
 *   	\file       smscommande/smscommandehistory_list.php
 *		\ingroup    commande
...
require_once '../lib/smsCommande.lib.php';
dol_include_once('/smscommande/class/smscommandehistory.class.php');
...
$search_status_commande=GETPOST('search_status_commande','alpha');
...
//Extrafield Statut
$extrafieldstatut = new ExtraFields($db);
$extrafieldstatut->fetch_name_optionals_label('commande');
$param_suivi=$extrafieldstatut->attribute_param['suivi'];
...
$hookmanager->initHooks(array('smscommandehistorylist'));
$extrafields = new ExtraFields($db);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('smscommande');
$search_array_options=$extrafields->getOptionalsFromPost($extralabels,'','search_');



// Load object if id or ref is provided as parameter
$object=new Smscommandehistory($db);
...
't.fk_commande'=>array('label'=>$langs->trans("Order"), 'checked'=>1),
't.fk_user'=>array('label'=>$langs->trans("User"), 'checked'=>1),
't.status_commande'=>array('label'=>$langs->trans("StatusSMS"), 'checked'=>1),
't.num_envoi'=>array('label'=>$langs->trans("PhoneShort"), 'checked'=>1),
't.content'=>array('label'=>$langs->trans("Message"), 'checked'=>1),
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/smscommande/list.php',1));
			exit;
		}
		else
		{
...
$help_url='';
$title = $langs->trans('smsCommandeHistory');

$page_name = "smsCommandeHistory";
llxHeader('', $langs->trans($page_name),'','','','','','',0,0);

// Configuration header
$head = smsCommandeAdminPrepareHead();
dol_fiche_head(
	$head,
	'historysms',
	$langs->trans("Module432412Name"),
	0,
	"smscommande@smscommande"
);
...
$sql.= " FROM ".MAIN_DB_PREFIX."smscommande_history as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."smscommande_history_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
...
if ($search_status_commande) $sql.= natural_search("status_commande",$search_status_commande);
...
if ($search_fk_commande != '') $params.= '&amp;search_fk_commande='.urlencode($search_fk_commande);
if ($search_fk_user != '') $params.= '&amp;search_fk_user='.urlencode($search_fk_user);
if ($search_status_commande != '') $params.= '&amp;search_status_commande='.urlencode($search_status_commande);
...
if (! empty($arrayfields['t.status_commande']['checked'])) print_liste_field_titre($arrayfields['t.status_commande']['label'],$_SERVER['PHP_SELF'],'t.status_commande','',$params,'',$sortfield,$sortorder);
...
if (! empty($arrayfields['t.status_commande']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_status_commande" value="'.$search_status_commande.'" size="10"></td>';
...
if (! empty($arrayfields['t.content']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_content" value="'.$search_content.'" size="10"></td>';
    ...
    while ($i < min($num, $limit))
    {
        ...
        if ($obj)
        {
            ...
            print '<tr '.$bc[$var].'>';
            if (! empty($arrayfields['t.fk_commande']['checked'])) 
            {
                require_once DOL_DOCUMENT_ROOT . '/commande/class/commande.class.php';
                $commande = new Commande($db);
                $commande->fetch($obj->fk_commande);
                print '<td><a href="'.DOL_URL_ROOT.'/commande/card.php?id='.$obj->fk_commande.'">'.$commande->ref.'</a></td>';
            ...
            if (! empty($arrayfields['t.status_commande']['checked'])) 
            {
                print '<td>'.$param_suivi['options'][$obj->status_commande].'</td>';
    		    if (! $i) $totalarray['nbfield']++;
            }
